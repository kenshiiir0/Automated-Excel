    ...
    await sendViaSmtp({
        ...
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto;">
                <h2 style="color:#0F5777;">GetMeds HR</h2>
                <p>Hi ${employeeName},</p>
                <p>${senderName ? `${senderName} from` : 'The'} GetMeds HR Department has shared a file with you: <strong>${attachmentFilename}</strong>.</p>
                ${note ? `<p style="background:#f7fafc; border-left:3px solid #1D9FDA; padding:10px 14px; margin:16px 0;">${note}</p>` : ''}
                <p style="color:#718096; font-size: 13px;">
