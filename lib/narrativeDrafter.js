...
Write 1-3 short, formal paragraphs (plain text, no markdown, no headers, no bullet points) stating these facts clearly and professionally, the way an HR department would write them in an official memo. Do not invent any facts not given above. Do not include a greeting, signature, or closing line -- only the narrative paragraph(s) themselves.`;
    ...
    for (const modelName of MODEL_CANDIDATES) {
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
            try {
                ...
                if (status === 429) break; // quota -- every candidate shares the
                    // same free-tier daily allowance, so trying the next
                    // model name won't help; stop burning through the
                    // list and surface the real quota error immediately.
    ...
    // Free-tier Gemini API keys are capped at a small number of requests
    // PER DAY (not per minute) -- once that's used up, every model
    // candidate returns the same 429 "quota exceeded" error, so retrying
    // or switching models can't help until the quota resets. Rather than
    // surface Google's raw JSON error body (a wall of nested objects
    // about quotaId/quotaDimensions/retryDelay that reads as a crash to
    // anyone who isn't debugging the API itself), detect this specific
    // case and replace it with a short, human message that says exactly
    // what happened and that it's expected, not broken.
    if (lastError && (lastError.status === 429 || /quota exceeded|too many requests/i.test(lastError.message || ''))) {
        const quotaErr = new Error(
            "Today's free AI drafting limit has been reached. This resets automatically -- try again later today or tomorrow, or type the narrative directly for now."
        );
