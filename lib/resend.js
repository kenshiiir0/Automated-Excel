...
// getmeds.ph is verified in Resend as of 2026-08-27 -- sending from the
// company domain now instead of Resend's shared onboarding@resend.dev
// testing address. This is the only place the sender address is defined.
const FROM_ADDRESS = 'GetMeds HR <noreply@getmeds.ph>';
...
// Sends a generated disciplinary memo (NTE / Written Warning / Final
// Written Warning) to the employee, with the .docx attached. replyToEmail
// is set to the HR user who clicked Send, NOT a generic company inbox --
// so the employee's reply lands directly in that person's mailbox, even
// though the email itself is sent through the shared Resend account
// rather than that person's own mailbox (per the agreed approach: no
// per-admin OAuth setup required).
export async function sendDisciplinaryMemoEmail({ toEmail, employeeName, memoLabel, replyToEmail, replyToName, attachmentBuffer, attachmentFilename }) {
    ...
    if (error) {
        throw new Error(error.message || 'Failed to send disciplinary memo email.');
    }
}
